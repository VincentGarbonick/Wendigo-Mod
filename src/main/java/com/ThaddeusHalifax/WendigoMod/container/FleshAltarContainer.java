package com.ThaddeusHalifax.WendigoMod.container;
...
public class FleshAltarContainer extends Container
{
    private final TileEntity tileEntity;
    private final PlayerEntity playerEntity;
    private final IItemHandler playerInventory;

    public FleshAltarContainer(int windowId, World world, BlockPos pos,
                               PlayerInventory playerInventory, PlayerEntity player)
    {
        super(RegistryHandler.FLESH_ALTAR_CONTAINER.get(), windowId);
        this.tileEntity = world.getBlockEntity(pos);
        playerEntity = player;
        this.playerInventory = new InvWrapper(playerInventory);
        layoutPlayerInventorySlots(8,86);

        System.out.print("pre not null\n");
        if(tileEntity != null)
        {
            System.out.print("not null \n");
            tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(h ->
            {
                System.out.print("adding slots? \n");
